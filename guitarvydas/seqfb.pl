
b(id12,425).
b(id13,395).
b(id14,410).
b(id15,510).
b(id16,480).
b(id17,495).
b(id18,410).
b(id19,510).
b(id20,510).
b(id21,495).
b(id22,480).
b(id23,495).
b(id24,495).
b(id25,425).
b(id26,395).
b(id27,410).
b(id28,410).
b(id29,395).
b(id30,410).
b(id31,425).
b(id32,455).
b(id33,410).
b(id34,410).
b(id35,485).
b(id36,470).
b(id37,480).
b(id38,495).
b(id39,525).
b(id4,630).
b(id40,510).
b(id41,495).
b(id42,495).
b(id43,555).
b(id44,540).
b(id5,425).
b(id6,425).
b(id7,410).
b(id8,395).
b(id9,410).
cell(id10,"").
cell(id11,"").
cell(id12,"").
cell(id13,"").
cell(id14,"").
cell(id15,"").
cell(id16,"").
cell(id17,"").
cell(id18,"").
cell(id19,"").
cell(id2,"").
cell(id20,"").
cell(id21,"").
cell(id22,"").
cell(id23,"").
cell(id24,"").
cell(id25,"").
cell(id26,"").
cell(id27,"").
cell(id28,"").
cell(id29,"").
cell(id3,"").
cell(id30,"").
cell(id31,"").
cell(id32,"").
cell(id33,"").
cell(id34,"").
cell(id35,"").
cell(id36,"").
cell(id37,"").
cell(id38,"").
cell(id39,"").
cell(id4,"").
cell(id40,"").
cell(id41,"").
cell(id42,"").
cell(id43,"").
cell(id44,"").
cell(id5,"").
cell(id6,"").
cell(id7,"").
cell(id8,"").
cell(id9,"").
contains(id12,id14).
contains(id15,id17).
contains(id19,id23).
contains(id19,id24).
contains(id20,id21).
contains(id20,id38).
contains(id25,id27).
contains(id25,id28).
contains(id31,id33).
contains(id31,id34).
contains(id35,id36).
contains(id4,id12).
contains(id4,id13).
contains(id4,id15).
contains(id4,id16).
contains(id4,id19).
contains(id4,id20).
contains(id4,id22).
contains(id4,id25).
contains(id4,id26).
contains(id4,id29).
contains(id4,id31).
contains(id4,id32).
contains(id4,id35).
contains(id4,id37).
contains(id4,id39).
contains(id4,id40).
contains(id4,id43).
contains(id4,id5).
contains(id4,id6).
contains(id4,id8).
contains(id40,id41).
contains(id40,id42).
contains(id43,id44).
contains(id5,id18).
contains(id5,id9).
contains(id6,id30).
contains(id6,id7).
contains1(id4,id12).
contains1(id4,id13).
contains1(id4,id15).
contains1(id4,id16).
contains1(id4,id19).
contains1(id4,id20).
contains1(id4,id22).
contains1(id4,id25).
contains1(id4,id26).
contains1(id4,id29).
contains1(id4,id31).
contains1(id4,id32).
contains1(id4,id35).
contains1(id4,id37).
contains1(id4,id39).
contains1(id4,id40).
contains1(id4,id43).
contains1(id4,id5).
contains1(id4,id6).
contains1(id4,id8).
diagram(id1, "").
diagramContains(id1, id10).
diagramContains(id1, id11).
diagramContains(id1, id12).
diagramContains(id1, id13).
diagramContains(id1, id14).
diagramContains(id1, id15).
diagramContains(id1, id16).
diagramContains(id1, id17).
diagramContains(id1, id18).
diagramContains(id1, id19).
diagramContains(id1, id2).
diagramContains(id1, id20).
diagramContains(id1, id21).
diagramContains(id1, id22).
diagramContains(id1, id23).
diagramContains(id1, id24).
diagramContains(id1, id25).
diagramContains(id1, id26).
diagramContains(id1, id27).
diagramContains(id1, id28).
diagramContains(id1, id29).
diagramContains(id1, id3).
diagramContains(id1, id30).
diagramContains(id1, id31).
diagramContains(id1, id32).
diagramContains(id1, id33).
diagramContains(id1, id34).
diagramContains(id1, id35).
diagramContains(id1, id36).
diagramContains(id1, id37).
diagramContains(id1, id38).
diagramContains(id1, id39).
diagramContains(id1, id4).
diagramContains(id1, id40).
diagramContains(id1, id41).
diagramContains(id1, id42).
diagramContains(id1, id43).
diagramContains(id1, id44).
diagramContains(id1, id5).
diagramContains(id1, id6).
diagramContains(id1, id7).
diagramContains(id1, id8).
diagramContains(id1, id9).
edge(id10, "").
edge(id11, "").
edge(id13, "").
edge(id16, "").
edge(id22, "").
edge(id26, "").
edge(id29, "").
edge(id32, "").
edge(id37, "").
edge(id39, "").
edge(id8, "").
ellipse(id14, "").
ellipse(id17, "").
ellipse(id18, "").
ellipse(id21, "").
ellipse(id23, "").
ellipse(id24, "").
ellipse(id27, "").
ellipse(id28, "").
ellipse(id30, "").
ellipse(id33, "").
ellipse(id34, "").
ellipse(id36, "").
ellipse(id38, "").
ellipse(id41, "").
ellipse(id42, "").
ellipse(id44, "").
ellipse(id7, "").
ellipse(id9, "").
fillColor(id14, "yellow").
fillColor(id17, "yellow").
fillColor(id18, "green").
fillColor(id21, "green").
fillColor(id23, "yellow").
fillColor(id24, "green").
fillColor(id27, "yellow").
fillColor(id28, "green").
fillColor(id30, "yellow").
fillColor(id33, "yellow").
fillColor(id34, "green").
fillColor(id36, "green").
fillColor(id38, "yellow").
fillColor(id41, "yellow").
fillColor(id42, "green").
fillColor(id44, "green").
fillColor(id7, "green").
fillColor(id9, "yellow").
height(id12, 60).
height(id14, 30).
height(id15, 60).
height(id17, 30).
height(id18, 30).
height(id19, 60).
height(id20, 60).
height(id21, 30).
height(id23, 30).
height(id24, 30).
height(id25, 60).
height(id27, 30).
height(id28, 30).
height(id30, 30).
height(id31, 60).
height(id33, 30).
height(id34, 30).
height(id35, 60).
height(id36, 30).
height(id38, 30).
height(id4, 340).
height(id40, 60).
height(id41, 30).
height(id42, 30).
height(id43, 60).
height(id44, 30).
height(id5, 60).
height(id6, 60).
height(id7, 30).
height(id9, 30).
l(id12,280).
l(id13,395).
l(id14,380).
l(id15,280).
l(id16,395).
l(id17,380).
l(id18,480).
l(id19,490).
l(id20,700).
l(id21,690).
l(id22,605).
l(id23,590).
l(id24,480).
l(id25,890).
l(id26,1005).
l(id27,990).
l(id28,880).
l(id29,815).
l(id30,800).
l(id31,1080).
l(id32,1235).
l(id33,1220).
l(id34,1070).
l(id35,1360).
l(id36,1350).
l(id37,835).
l(id38,820).
l(id39,950).
l(id4,260).
l(id40,890).
l(id41,990).
l(id42,880).
l(id43,1365).
l(id44,1355).
l(id5,490).
l(id6,700).
l(id7,690).
l(id8,605).
l(id9,590).
name(id1, "sequence").
portdirection(id14,output).
portdirection(id17,output).
portdirection(id18,input).
portdirection(id21,input).
portdirection(id23,output).
portdirection(id24,input).
portdirection(id27,output).
portdirection(id28,input).
portdirection(id30,output).
portdirection(id33,output).
portdirection(id34,input).
portdirection(id36,input).
portdirection(id38,output).
portdirection(id41,output).
portdirection(id42,input).
portdirection(id44,input).
portdirection(id7,input).
portdirection(id9,output).
r(id12,400).
r(id13,495).
r(id14,410).
r(id15,400).
r(id16,495).
r(id17,410).
r(id18,510).
r(id19,610).
r(id20,840).
r(id21,720).
r(id22,705).
r(id23,620).
r(id24,510).
r(id25,1010).
r(id26,1085).
r(id27,1020).
r(id28,910).
r(id29,895).
r(id30,830).
r(id31,1240).
r(id32,1365).
r(id33,1250).
r(id34,1100).
r(id35,1520).
r(id36,1380).
r(id37,895).
r(id38,850).
r(id39,1425).
r(id4,1560).
r(id40,1010).
r(id41,1020).
r(id42,910).
r(id43,1525).
r(id44,1385).
r(id5,610).
r(id6,820).
r(id7,720).
r(id8,705).
r(id9,620).
rect(id12,"").
rect(id15,"").
rect(id19,"").
rect(id2,"").
rect(id20,"").
rect(id25,"").
rect(id3,"").
rect(id31,"").
rect(id35,"").
rect(id4,"").
rect(id40,"").
rect(id43,"").
rect(id5,"").
rect(id6,"").
source(id13, id14).
source(id16, id17).
source(id22, id23).
source(id26, id27).
source(id29, id30).
source(id32, id33).
source(id37, id38).
source(id39, id40).
source(id8, id9).
t(id12,365).
t(id13,395).
t(id14,380).
t(id15,450).
t(id16,480).
t(id17,465).
t(id18,380).
t(id19,450).
t(id20,450).
t(id21,465).
t(id22,480).
t(id23,465).
t(id24,465).
t(id25,365).
t(id26,395).
t(id27,380).
t(id28,380).
t(id29,395).
t(id30,380).
t(id31,365).
t(id32,395).
t(id33,380).
t(id34,380).
t(id35,425).
t(id36,440).
t(id37,480).
t(id38,465).
t(id39,480).
t(id4,290).
t(id40,450).
t(id41,465).
t(id42,465).
t(id43,495).
t(id44,510).
t(id5,365).
t(id6,365).
t(id7,380).
t(id8,395).
t(id9,380).
target(id13, id18).
target(id16, id24).
target(id22, id21).
target(id26, id34).
target(id29, id28).
target(id32, id36).
target(id37, id42).
target(id39, id43).
target(id8, id7).
value(id12, "buildSequence").
value(id14, "").
value(id15, "buildDetails").
value(id17, "").
value(id18, "").
value(id19, "detailsStart").
value(id20, "detailsEmitJSON").
value(id21, "").
value(id23, "").
value(id24, "").
value(id25, "emitTopo").
value(id27, "").
value(id28, "").
value(id30, "").
value(id31, "topologicalSort").
value(id33, "").
value(id34, "").
value(id35, "combine").
value(id36, "").
value(id38, "").
value(id4, "topLevel").
value(id40, "emitFunctions").
value(id41, "").
value(id42, "").
value(id43, "combine").
value(id44, "").
value(id5, "sequenceStart").
value(id6, "seqEmitJSON").
value(id7, "").
value(id9, "").
vertex(id12, "").
vertex(id14, "").
vertex(id15, "").
vertex(id17, "").
vertex(id18, "").
vertex(id19, "").
vertex(id20, "").
vertex(id21, "").
vertex(id23, "").
vertex(id24, "").
vertex(id25, "").
vertex(id27, "").
vertex(id28, "").
vertex(id30, "").
vertex(id31, "").
vertex(id33, "").
vertex(id34, "").
vertex(id35, "").
vertex(id36, "").
vertex(id38, "").
vertex(id4, "").
vertex(id40, "").
vertex(id41, "").
vertex(id42, "").
vertex(id43, "").
vertex(id44, "").
vertex(id5, "").
vertex(id6, "").
vertex(id7, "").
vertex(id9, "").
width(id12, 120).
width(id14, 30).
width(id15, 120).
width(id17, 30).
width(id18, 30).
width(id19, 120).
width(id20, 140).
width(id21, 30).
width(id23, 30).
width(id24, 30).
width(id25, 120).
width(id27, 30).
width(id28, 30).
width(id30, 30).
width(id31, 160).
width(id33, 30).
width(id34, 30).
width(id35, 160).
width(id36, 30).
width(id38, 30).
width(id4, 1300).
width(id40, 120).
width(id41, 30).
width(id42, 30).
width(id43, 160).
width(id44, 30).
width(id5, 120).
width(id6, 120).
width(id7, 30).
width(id9, 30).
x(id12, 280).
x(id14, 380).
x(id15, 280).
x(id17, 380).
x(id18, 480).
x(id19, 490).
x(id20, 700).
x(id21, 690).
x(id23, 590).
x(id24, 480).
x(id25, 890).
x(id27, 990).
x(id28, 880).
x(id30, 800).
x(id31, 1080).
x(id33, 1220).
x(id34, 1070).
x(id35, 1360).
x(id36, 1350).
x(id38, 820).
x(id4, 260).
x(id40, 890).
x(id41, 990).
x(id42, 880).
x(id43, 1365).
x(id44, 1355).
x(id5, 490).
x(id6, 700).
x(id7, 690).
x(id9, 590).
y(id12, 365).
y(id14, 380).
y(id15, 450).
y(id17, 465).
y(id18, 380).
y(id19, 450).
y(id20, 450).
y(id21, 465).
y(id23, 465).
y(id24, 465).
y(id25, 365).
y(id27, 380).
y(id28, 380).
y(id30, 380).
y(id31, 365).
y(id33, 380).
y(id34, 380).
y(id35, 425).
y(id36, 440).
y(id38, 465).
y(id4, 290).
y(id40, 450).
y(id41, 465).
y(id42, 465).
y(id43, 495).
y(id44, 510).
y(id5, 365).
y(id6, 365).
y(id7, 380).
y(id9, 380).
